...
public class MaskedImageSet {
    ...
    public function addFromBitmapData(images:BitmapData, masks:BitmapData, width:int, height:int):void {
        ...
        for (var i:int = 0; i < imagesSet.images.length; i++) {
            this.images_.push(new MaskedImage(imagesSet.images[i],
                    (masksSet == null || masksSet.images.length <= i) ? null : masksSet.images[i]));
        }
    }
