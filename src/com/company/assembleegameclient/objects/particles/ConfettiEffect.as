...
public class ConfettiEffect extends ParticleEffect {
    ...
    public const colors:Array = [65535, 16711935, 16776960, 16777215];
    ...
    override public function update(param1:int, param2:int):Boolean {
        var _loc5_:int = 0;
        var _loc4_:ConfettiParticle = null;
        x_ = this.start_.x;
        y_ = this.start_.y;
        var _loc6_:int = 0;
        while (_loc6_ < 5) {
            if (this.confettiColor_) {
                _loc5_ = colors[int(Math.random() * 5)];
            } else {
                _loc5_ = this.color_;
            }
            _loc4_ = new ConfettiParticle(1.85, (3 + int(Math.random() * 5)) * 20, _loc5_, 500 * this.lifetime_ + Math.random() * 500 * this.lifetime_, 0.1 + Math.random() * 0.1, this.start_, this.end_);
